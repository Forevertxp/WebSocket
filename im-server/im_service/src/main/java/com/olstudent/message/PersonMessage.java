...
import com.olstudent.pojo.Message;
...
public class PersonMessage {
    ...
    private void offline() {
        ...
        try {
            ...
            msgDao.connect();
            // 查询是否存在该用户
            List<User> users = userDao.queryByPhone(receiver);
            if (users.size() == 1) {
                int row = msgDao
     ...
     * @return
     */
    private void sendResponse(int status, String body) {
        MessageHolder messageHolder = new MessageHolder();
        ...
        String jsonString = JSONObject.toJSONString(messageHolder);
        TextWebSocketFrame tws = new TextWebSocketFrame(jsonString);
        ChannelFuture future = channel.writeAndFlush(tws);
        future.addListener(new ChannelFutureListener() {
            @Override
            public void operationComplete(ChannelFuture future) throws Exception {
                if (!future.isSuccess()) {
