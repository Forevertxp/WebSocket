...
import com.olstudent.pojo.OfflineMessage;
...
public class MsgDao extends JdbcConn {
    ...
    public int insertMsg(String sender, String receiver, String msg, Timestamp time) {
        String sql = "INSERT INTO im_offline_msg (sender, receiver, message, time) VALUES (?, ?, ?, ?)";
        int row = 0;
        try {
    ...
    public int removeMsg(String receiver) {
        String sql = "DELETE FROM im_offline_msg where receiver = ?";
        int row = 0;
        try {
            pstmt = conn.prepareStatement(sql);
    ...
    public List<OfflineMessage> queryMsg(String receiver) {
        ...
        String sql = "SELECT * FROM im_offline_msg WHERE receiver = ?";
        try {
            pstmt = conn.prepareStatement(sql);
            pstmt.setString(1, receiver);
