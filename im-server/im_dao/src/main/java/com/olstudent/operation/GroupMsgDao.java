...
import com.olstudent.pojo.OfflineGroupMessage;
...
public class GroupMsgDao extends JdbcConn {
    ...
    public int insertMsg(String sender,String receiver, String name, String msg, Timestamp time) {
        String sql = "INSERT INTO im_offline_msg_group (sender,receiver, name, message, time) VALUES (?, ?, ?, ?, ?)";
        int row = 0;
        try {
    ...
    public int removeMsg(String receiver) {
        String sql = "DELETE FROM im_offline_msg_group where receiver = ?";
        int row = 0;
        try {
            pstmt = conn.prepareStatement(sql);
    ...
    public List<OfflineGroupMessage> queryMsg(String receiver) {
        ...
        String sql = "SELECT * FROM im_offline_msg_group WHERE receiver = ?";
        try {
            pstmt = conn.prepareStatement(sql);
            pstmt.setString(1, receiver);
